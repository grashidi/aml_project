...
model_name = "densenet121_e10_bs10_10-09-2021_14:48:30.pt"
...
densenet121.classifier.register_forward_hook(get_activations("classifier"))
densenet121.classifier.register_backward_hook(get_grads("classifier"))
...
for batch_index, batch_samples in tqdm(enumerate(test_loader)):
    ...
    activations = activations["classifier"]
    grads = grads["classifier"][0]

    grads = torch.mean(grads, dim=(-2, -1), keepdims=True) # gradients are pooled
    grad_cam = activations * grads
    grad_cam = grad_cam.unsqueeze(dim=-3)
